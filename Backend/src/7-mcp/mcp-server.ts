...
type SearchVacationsArgs = {
    destination?: string | undefined;
    status?: VacationStatus | undefined;
    minPrice?: number | undefined;
    maxPrice?: number | undefined;
    sortBy?: VacationSortBy | undefined;
...
class McpServerBuilder {
    ...
    private readonly searchVacationsSchema = z.object({
    ...
    public createMcpServer(): McpServer {
        const mcpServer = new McpServer(
            ...
            {
                instructions: "Use these tools only for questions about the vacations database."
            }
        );

        this.registerSearchVacationsTool(mcpServer);
        this.registerVacationsStatsTool(mcpServer);

        return mcpServer;
    }

    private registerSearchVacationsTool(mcpServer: McpServer): void {
        mcpServer.registerTool(
            "search_vacations",
            {
                title: "Search Vacations",
                description: "Returns vacations from the database with likes count.",
                inputSchema: this.searchVacationsSchema
            },
            async (args) => {
                return this.searchVacations(args);
            }
        );
    }

    private registerVacationsStatsTool(mcpServer: McpServer): void {
        mcpServer.registerTool(
            "vacations_stats",
            {
            ...
            async () => {
                return this.getVacationsStats();
            }
        );
    }

    private async searchVacations(args: SearchVacationsArgs) {
        ...
        const vacations = await dal.execute(sql, values) as VacationRow[];
        ...
        return {
            content: [
                { type: "text" as const, text: JSON.stringify(normalizedVacations) }
            ],
            structuredContent: { vacations: normalizedVacations }
        };
    }

    private async getVacationsStats() {
        const today = new Date().toISOString().slice(0, 10);
        ...
        const result = await dal.execute(sql, [today, today, today, today]) as StatsRow[];
